...
for i=0.d0,nxn-2 do begin
   ...
   for p=0.d0,nzn-2 do begin
      ...
      neg=WHERE(rotvelor LT 0.D0)
      IF neg[0] GE 0. then rotvel[neg]=rotvelor[neg]+(lag*(abs(z)))
      pos=WHERE(rotvelor GT 0.D0)
      IF pos[0] GE 0. then  rotvel[pos]=rotvelor[pos]-(lag*(abs(z)))
      zero=WHERE(rotvelor EQ 0.D0)
      IF zero[0] GE 0. then rotvel[zero]=0.D0
...
Pro Func3,f2,t,y,z,R0,z0,sini,cosi,td,tau,csttau,$
  ...
  vadd=dblarr(nv)
  vel=dblarr(nv)
  vel[*]=vlow+findgen(nv)*vscale
  vadd[*]=double(EXP(-5.D-1*(((vel[*]-result)/dveldisp)^2.D0)))
  test=0.D0
  test=TOTAL(vadd)
  vadd[*]=vadd[*]*f2d/test
  hulp=dblarr(nv)
  hulp[*]=double(EXP(-5.D-1*(((vel[*]-result)/buldisp)^2.)))
  test=0.D0
  test=TOTAL(hulp)
...
for i=0.d0,NN-1 do begin
   ...
   for k=0.d0,15.D0 do begin
      if (XX[k] le (-!dpi/2.D0+1.D-4)) or $
         ...
         (sqrt(y*y+(z/cosi+TAN(XX[k])*sini)*(z/cosi+TAN(XX[k])*sini)) gt trRdi) then Sum2=double(Sum2+0.D0) else begin
         tt=double(TAN(XX[k]))
